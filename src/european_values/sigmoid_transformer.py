...
class SigmoidTransformer:
    ...
    def fit(self, X: "ndarray") -> "SigmoidTransformer":
        ...
        lower = self.quantile(X, q=0.01)
        upper = self.quantile(X, q=0.99)
        ...
        result: opt.OptimizeResult = opt.minimize(
            fun=partial(self._loss, array=X, target=0.9, alpha=self.alpha_), x0=[0.0]
        )
    ...
    def transform(self, X: "ndarray") -> "ndarray":
        ...
        """
        if not hasattr(self, "alpha_") or not hasattr(self, "center_"):
            raise ValueError(
                "SigmoidTransformer must be fitted before calling transform."
            )
        return sigmoid(self.alpha_ * (X - self.center_))

    @staticmethod
    def quantile(X: "ndarray", q: float) -> float:
        """Calculate the quantile of the input array.
        ...
        Raises:
            ValueError:
                If the quantile is not between 0 and 1.
        """
        if not (0 <= q <= 1):
            raise ValueError("Quantile must be between 0 and 1.")
        sorted_values = sorted(list(X))
