...
In the `src` directory there are two subdirectories, `european_values`
and `scripts`. This is a brief explanation of the differences between the two.

### Modules
All Python files in the `european_values` directory are _modules_
...
```
from european_values import some_function
```
...
`alexandrainst/european_values` then simply press the `<> Code` button
